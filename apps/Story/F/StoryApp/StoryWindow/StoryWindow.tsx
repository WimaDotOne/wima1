...
export function StoryWindow({
  ...
  function goToView(viewId: string) {
    router.push(`/apps/Story/AppTurn/${viewId}`)
  }
  ...
  const storyMenu = StoryMenu(win?.viewId)

  return(<>
    <AppleWindow menu={storyMenu} brand="Story"
...
function IsViewRequreLogin(viewId?: string) {
  switch (viewId) {
    case AppTurn.MyStory: return true
    case AppTurn.Projects: return true
    default: return false
  }
}

function StoryMenu(viewId?: string): MenuModel | undefined {
  if(!viewId) viewId = AppTurn.MyStory
  const myGroup = new GroupModel("My Stories", false)
  myGroup.AddItem(new ItemModel(AppTurn.MyStory, "Stories", "book", true))
  myGroup.AddItem(new ItemModel(AppTurn.Projects, "Projects", "typewriter", true))

  const publicGroup = new GroupModel("Story", true)
  publicGroup.AddItem(new ItemModel(AppTurn.About, "About", "smile"))
  publicGroup.AddItem(new ItemModel(AppTurn.Tutorial, "How to publish a story ?", "graduationcap"))
  publicGroup.AddItem(new ItemModel(AppTurn.WimaCircle, "Exit", "wimacircle")) 

  const menu = new MenuModel(viewId, StoryColor.themeGreen)
